...
class SearchListInfoPresenter(view: ListInfoContract.View) : BaseListInfoPresenter(view) {

    override fun onRefresh() {
        unSubscribe()
        mView.onRefresh(true)
        val subscription = DataManager.handleNormalService<ArsenalListInfo>(
            DataManager.getArsenalApi().search(
                String.format(
                    "/search?q=%s",
                    mCurrentKey
                )
    ...
    override fun onLoadMore() {
        ...
        unSubscribe()
        val subscription =
            DataManager.handleNormalService<ArsenalListInfo>(DataManager.getArsenalApi().search(mHasMore), { info ->
                mHasMore = info.hasMore
